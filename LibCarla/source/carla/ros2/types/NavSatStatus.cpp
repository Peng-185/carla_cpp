        ...
        size_t current_alignment)
{
    ...
    current_alignment += 1 + eprosima::fastcdr::Cdr::alignment(current_alignment, 1); // 先增加1字节（可能是某种头部或标识信息等），然后根据当前对齐字节数对增加后的位置进行对齐调整（保证符合CDR格式要求的对齐方式）
    current_alignment += 2 + eprosima::fastcdr::Cdr::alignment(current_alignment, 2);// 再增加2字节（可能是另一部分数据等），同样进行对齐调整

    return current_alignment - initial_alignment;// 返回计算出的序列化后相对于初始对齐位置增加的字节数，即实际序列化占用的大小
}
